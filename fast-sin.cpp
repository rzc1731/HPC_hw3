...
void sin4_taylor_extra(double* sinx, const double* x) {
  for (int i = 0; i < 4; i++) {
    int det1 = floor((x[i]+M_PI/4)/(M_PI/2));
    double x_in = x[i] - det1 * M_PI / 2;
    int det2 = floor((x[i]+M_PI/4)/M_PI);
...
void sin4_intrin_extra(double* sinx, const double* x) {
  // The definition of intrinsic functions can be found at:
  // https://software.intel.com/sites/landingpage/IntrinsicsGuide/#
#if defined(__AVX__)
  ...
  for (int i = 0; i < 4; i++) {
    det1[i] = floor((x[i]+M_PI/4)/(M_PI/2));
    x_in[i] = x[i] - det1[i] * M_PI / 2;
    det2[i] = floor((x[i]+M_PI/4)/M_PI);
  }

  __m256d x1, x2, x3, x5, x7, x9, x11;
  x1  = _mm256_load_pd(x_in);
  x2  = _mm256_mul_pd(x1, x1);
  x3  = _mm256_mul_pd(x1, x2);
  x5  = _mm256_mul_pd(x3, x2);
  x7  = _mm256_mul_pd(x5, x2);
  ...
  for (int i = 0; i < 4; i++) {
    int mod1 = (det1[i] % 2 + 2) % 2;
    int mod2 = (det2[i] % 2 + 2) % 2;
    if (mod1 == 0) { // sin
      if (mod2 == 1) { // -sinx
        sinx[i] *= -1;
      }
...
int main() {
  ...
  double* sinx_taylor_extra = (double*) aligned_malloc(N*sizeof(double));
  double* sinx_intrin = (double*) aligned_malloc(N*sizeof(double));
  double* sinx_intrin_extra = (double*) aligned_malloc(N*sizeof(double));
  double* sinx_vector = (double*) aligned_malloc(N*sizeof(double));
  for (long i = 0; i < N; i++) {
    //x[i] = (drand48()-0.5) * M_PI/2; // [-pi/4,pi/4]
    x[i] = (drand48()-0.5) * 4*M_PI; // [-2pi,2pi] test for extra credit
    sinx_ref[i] = 0;
    sinx_taylor[i] = 0;
    sinx_taylor_extra[i] = 0;
    sinx_intrin[i] = 0;
    sinx_intrin_extra[i] = 0;
    sinx_vector[i] = 0;
  }
  ...
  for (long rep = 0; rep < 1000; rep++) {
    for (long i = 0; i < N; i+=4) {
      sin4_taylor_extra(sinx_taylor_extra+i, x+i);
    }
  }
  printf("Taylor Extra time:    %6.4f      Error: %e\n", tt.toc(), err(sinx_ref, sinx_taylor_extra, N));
  ...
  for (long rep = 0; rep < 1000; rep++) {
    for (long i = 0; i < N; i+=4) {
      sin4_intrin_extra(sinx_intrin_extra+i, x+i);
    }
  }
  printf("Intrin Extra time:    %6.4f      Error: %e\n", tt.toc(), err(sinx_ref, sinx_intrin_extra, N));
  ...
  aligned_free(sinx_taylor_extra);
  aligned_free(sinx_intrin);
  aligned_free(sinx_intrin_extra);
